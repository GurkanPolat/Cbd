...
jQuery(document).ready(function($) {
    ...
    // var elmt = $( '.timeline-navigation li.active' );
    //
    // $( '.timeline-navigation li' ).hover(
    //   function() {
    //     elmt.removeClass( 'active' );
    //     $( this ).addClass( 'active' );
    //     //$( this ).prev().addClass( 'active-neighbor' );
    //     //$( this ).next().addClass( 'active-neighbor' );
    //     //$( this ).switchClass( "inactive", "active", 1000, "easeInOutQuad" );
    //   }, function() {
    //     $( this ).removeClass( 'active' );
    //     //$( this ).prev().removeClass( 'active-neighbor' );
    //     //$( this ).next().removeClass( 'active-neighbor' );
    //   }
    // );

    $(" .path-experiences .node--type-experience ").each(function() {
        var text = $(this).find(" .node__meta .inline-list__item ").html();
        if (/actuellement/i.test(text)) {
            $(this).addClass("current-exp");
        }
    });

    $(" .view-recettes .views-field-field-difficulte .field-content ").each(function() {
        var intRegex = /[0-9]+/;
        var text = $(this).html();
        var regexpArray = intRegex.exec(text);
        var star = "";
        if (regexpArray != null && regexpArray[0]) {
            for (var i = 0; i < 5; i++) {
                if (i < regexpArray[0]) {
    ...
    /* magnific popup gallery on portfolio */
    $(" .node--type-portfolio, .node--type-module ").each(function() {
        $('.mfp-field').magnificPopup({
            type: 'image',
            gallery: {
                enabled: true
            }
